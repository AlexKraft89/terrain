...
World = function(){
	...
	this.planes = [];
	 ...
	 this.save.saveterrian = function(object){
	 	...
	 	terrian.position = {"x":object.position.x - parseFloat(_cube[0]*2000) ,
	 					  "y":object.position.y,
		...
		$.post('/php/script/save.php',{'action':'SaveTerrain',
									   'name':object.name,
									   'string':string,
									   'x':_cube[0],
									   'z':_cube[1]},
	 ...
	 this.save.collision = function(object){
	 	var model = {};
	 	...
	 	model.depth =object.depth;
	 	model.heightSegments =object.heightSegments;
	 	model.widthSegments =object.widthSegments;
	 	model.depthSegments =object.depthSegments;
...
drewcude = function(object){
	...
	window.tmap = [ parseInt(x-1)+'='+parseInt(z+1) , x+'='+parseInt(z+1) , parseInt(x+1)+'='+parseInt(z+1) , 
			  	   parseInt(x-1)+'='+parseInt(z)    , x+'='+z   		  , parseInt(x+1)+'='+parseInt(z)   ,  
			 	   parseInt(x-1)+'='+parseInt(z-1)  , x+'='+parseInt(z-1) , parseInt(x+1)+'='+parseInt(z-1) ];		 	   
			    
//Теретория для моделей
    var delar = [];
...
/////////////////////
	...
	var ml = map.length;
	while(ml--){
			if( allterrain.indexOf( map[ml] ) === -1 ){
				ter.push(map[ml]);
			}
	}

	$.getJSON('/php/script/load.php',{'action':'getmap','ter':JSON.stringify(ter),'array':JSON.stringify(map)},function(responce){
