...
drewcude = function(object){
    ...
    while(sclen--){
		if (scene.children[sclen].cubIDs !== undefined){
			var mind = map.indexOf(scene.children[sclen].cubIDs);
			if (mind !== -1){
				map.splice(mind,1);
			}else {
	...
	while(i < l){
		...
		var p = this.planes.clone();
		var _p = 0;
		if (_coord[0] == 0 || _coord[1]== 0){
			_p = 500;
		}
		p.position.x = CUBSTEP * _coord[0];
		p.position.z = CUBSTEP * _coord[1];
		p.cubIDs = _coord[0] + '=' + _coord[1];
		scene.add(p);
		i++;
	}
