        ...
        Loader.animLoadStat(model.anim,
	                        ...
	                        {name:model.name,
	                         ...
	                         'anim':model.anim
	                        	...
	                        	var CUBSTEP = 2000;
                    			var _x=_z=CUBSTEP/2;
								if(morph.position.x < 0){
									_x = -_x;
								}
								if(morph.position.z < 0){
									_z = -_z;
								}

								var x = ~~(((_x+morph.position.x) / CUBSTEP));
								var z = ~~(((_z+morph.position.z) / CUBSTEP));
								morph.cubIDs = x+'='+z;
								var t = 100;
								morph.position.x = -1000;
								morph.position.z= -1000;
								while(t--){
									var newer = morph.clone();
									newer.position.x += Math.random()*10000;
									newer.position.z += Math.random()*10000;
		                            Game.scene.add( newer );
		                        }
